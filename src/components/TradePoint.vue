    ...
    async editPoint(reward) {
      if(reward.amount > 0){
        ...
        if(reward.exchange_point < user.point){
          ...
          this.$swal("Successful redemption", `You have successfully redeemed ${reward.reward_name}`, "success");
        }else{
          this.$swal("Rewards cannot be redeemed now", "Your points are not enough", "warning");
        }
      }else{
        this.$swal("Rewards cannot be redeemed now", "Rewards out of stock", "warning");
      }
    },
    async thisUser(){
